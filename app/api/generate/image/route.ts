 ...
 *
 * Headers:
 *   x-image-provider: ImageProviderId (default: 'siliconflow-image')
...
export async function POST(request: NextRequest) {
  try {
    ...
    log.error(
      `Image generation failed [provider=${request.headers.get('x-image-provider') ?? 'siliconflow-image'}, model=${request.headers.get('x-image-model') ?? 'default'}]:`,
      error,
    );
